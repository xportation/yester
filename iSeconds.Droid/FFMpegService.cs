...
namespace iSeconds.Droid
{
   ...
   public class FFMpegService : IntentService
   {
      ...
      protected override void OnHandleIntent(Intent intent)
      {
		...
		#if YESTER_LITE
		string getWatermarkFilePath(string basePath)
      {
			string file = "yester_watermark.png";
         ...
         if (new Java.IO.File(filename).Exists())
			...
			string yesterFile= System.IO.Path.Combine("yester_movie", file);
			var stream = Assets.Open(yesterFile);
		...
		bool addWatermark(string basePathAbsolute, string watermarkFilePath, string inputFile, string outputFile)
		{
			var cmd = string.Format(".{0} -loglevel error -i {1} -i {2} -strict -2 -filter_complex overlay=10:10 -qscale 1 -y {3}",
				System.IO.Path.Combine(basePathAbsolute, "ffmpeg"), inputFile, watermarkFilePath, outputFile);
      ...
      {
			#if YESTER_LITE
			string fileResult = outputFilePath;
			outputFilePath= System.IO.Path.Combine(basePathAbsolute, "temporary_lite_file.mp4");
			#endif
               ...
               if (videoFiles[i].Rotation != 0) {
                  ...
                  case 270:
                     ...
                     break;
						...
						var cmd = string.Format(".{0} -loglevel error -y -i {1} -vf " +
         ...
         if (!errors) {
            // -loglevel debug
				var command = string.Format(".{0} -loglevel error -y -f concat -i {1} -vf subtitles='{2}' -strict -2 -c:v mpeg4 " +
			...
			#if YESTER_LITE
			string watermarkFilePath= getWatermarkFilePath(basePathAbsolute);
			errors= !addWatermark(basePathAbsolute, watermarkFilePath, outputFilePath, fileResult);
			temporaryFiles.Add(outputFilePath);
			outputFilePath= fileResult;
			#endif
      ...
      {
         //-loglevel debug
			return string.Format(".{0} -loglevel error -y -i {1} -vf " +
